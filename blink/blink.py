...
with open('./config/privilege.json', 'r')as privilege:
    privilege = json.load(privilege)

with open('./config/ban.json', 'r')as ip_list:
...
def block():
    ...
    if ip not in privilege and request.url.split('/')[-1] != 'favicon.ico':
        ...
        if ip not in ip_timing.keys():
            ...
            if time.time()-ip_timing[ip] < 1:  # (random.randrange(0, 1)/10):

                if ip not in ip_ban_list:

                    '''request too fast and not in ban list->add to ban list'''
                    with open('./config/ban.json', 'w')as ip_list:
                        ip_ban_list.append(ip)
                        json.dump(ip_ban_list, ip_list)

                    '''create banning log'''
                    with open('./config/ban_log.txt', 'a')as log:
...
def login():
    ...
    with open('./config/admin.json')as admin:
    ...
    if hashlib.md5(user.encode()).hexdigest() in admin:
        ...
        if ip not in privilege:
            ...
            '''log and update'''
            with open('./config/server_log.txt', 'a')as normal_log:
                normal_log.write('adding new admin: %s @%s\n' %
                                 (ip, time.ctime(time.time())))

            with open('./config/privilege.json', 'w')as privilege_list:
        ...
        with open('./config/server_log.txt', 'a')as log:
            log.write('%s tried login but failed @%s' % (
                request.environ.get('REMOTE_ADDR'), time.ctime(time.time())))
...
def clear_up():
    ...
    try:
        ...
        with open('./config/except.txt', 'a')as except_log:
            except_log.write(str(e)+'\t'+str(time.ctime(time.time()))+'\n')
...
def trans():
    '''in progress'''
    return render_template('transcedence.html')


@app.route('/transcendence_process', methods=['POST'])
def trans_process():
    '''process file and return GIF'''
    ...
    if len(files) != 0:
        for file in files:
            save_path = ''.join(['./temp/', file.filename])
            file.save(save_path)
            if transcendence(save_path) == False:
                last_file = max(glob('./trans_temp/*.gif'),
                ...
                last_file = max(glob('./temp/*.*'), key=os.path.getctime)
